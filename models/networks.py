...
class ResBlock(nn.Module):
    ...
    def __init__(self, ic=4, oc=4, kernel_size=3, dropout=0.5, use_dropout=False):
...
class Generator(nn.Module):
    ...
    def __init__(self, layers=3, filts=1024, kernel_size=3, channels=3, res_layers=3):
        ...
        # residual core
        operations = [
            ResBlock(ic=filts, oc=filts, use_dropout=False, kernel_size=3)
...
class ContStyleLoss(nn.Module):
    ...
    def __init__(
        self, vgg, style_im, ct_wgt, style_wgt, style_layer_ids, content_layer_id
    ):
        super().__init__()
    ...
    def forward(self, input_img, target_img):
        ...
        result_st = [
            gram_mse_loss(inp, targ) * self.style_wgt
            for inp, targ in zip(inp_feat, self.style_feat)
        ]
